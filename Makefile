...
APP      := quiz-service
BIN      := bin/$(APP).exe
DATABASE_URL ?= postgres://postgres:postgres@localhost:5432/quiz_db?sslmode=disable
PROJECT_ROOT := $(CURDIR)
export PROJECT_ROOT
...
ifndef name
	...
	docker compose run --rm quiz-migration create -ext sql -dir /migrations -seq $(name)
...
migrate-action:
	@docker compose run --rm quiz-migration \
		-path=/migrations/ \
